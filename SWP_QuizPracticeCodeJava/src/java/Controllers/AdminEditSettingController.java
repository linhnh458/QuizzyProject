...
public class AdminEditSettingController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (request.getParameter("quesNum") != null) {
        ...
        System.out.println("sid: "+settingid);
        System.out.println("duration: "+duration);
        int status = Integer.parseInt(request.getParameter("status"));
        String url = "editSetting?sid="+settingid;
        if (duration > 0 && duration < 121 && questionNum > 0 && questionNum < 61 && maxAns > 3 && maxAns < 11) {
            sdao.editSetting(settingid, duration, questionNum, status, maxAns, quizid);  
